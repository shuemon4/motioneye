...
Your scratchpad flags SHA1 being used for request signatures as a weakness 

security-analysis-20251218-1000

, but the plan doesn’t include a remediation. This matters because signature auth is one of motionEye’s core controls (and you’re leaning on it for CSRF-ish protection 
...
Your Phase 1.1 fix correctly moves away from shell=True by switching to argv lists 

security-optimization-plan-2025…

, but it also says “Apply shlex.quote() to all user-supplied paths” 
...
Your token bucket prototype stores attempts in an unbounded dict keyed by identifier 

security-optimization-plan-2025…

. That can be abused (send lots of requests with changing identifiers → memory growth).
...
Your safe_path_join() uses startswith(base_dir) checks 

security-optimization-plan-2025…

. This can be bypassed by prefix tricks (e.g., /base vs /base_evil) unless you do it carefully.
...
The scratchpad notes motionEye binds on 0.0.0.0:8765 by default 
...
security-optimization-plan-2025…
 ...
 helps, but the plan should also change defaults and document a recommended topology.
...
Your scratchpad flags that remote camera communication includes credentials in URLs 

security-analysis-20251218-1000

. That’s a big real-world leak vector (logs, browser history, referrers, proxies).
...
Your scratchpad lists motioneye/webhook.py as in-scope 

security-analysis-20251218-1000

, but the plan doesn’t mention it. Webhooks often become the “one URL that does privileged things.”
...
Encrypting secrets in config using Fernet 
...
file permissions + ownership + “secrets in env vars” first (you already do this for OAuth) 

security-optimization-plan-2025…

clear documentation about threat model (“protects against accidental disclosure / non-root reads”, not full device compromise)

9) Dependency/version requirements section is outdated and could mislead

Your plan says “OpenSSL 1.1.1+” 

security-optimization-plan-2025…

, but OpenSSL 1.1.1 reached EOL in 2023 
OpenSSL Corporation
. On modern Raspberry Pi OS releases, you’re likely to be on OpenSSL 3.x anyway.
...
Raspberry Pi
, because not all Raspberry Pi hardware can run 64-bit userlands 
Raspberry Pi Forums
+1
.
...
Secure boot expectations differ by Pi generation
...
There are community reports around ARM crypto extension behavior on Pi 4/5 
Raspberry Pi Forums
+1
. Whether or not that’s perfectly representative, the takeaway is:
